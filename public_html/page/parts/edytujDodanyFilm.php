<?php
	...
	if(isset($_POST['submit'])){
		...
		$flag = true;
		$error = 0;
		if(empty($name) || empty($description) || empty($year) || empty($image) || empty($url) || empty($filmewb) || empty($category) || empty($versionFilm)){
			$flag = false;
			$error = 1;}
		...
		if($flag == true){
			if($res2 = $Connect->query("UPDATE films SET name = '$name' , description = '$description' , data = '$year' ,  image = '$image' , link = '$url' , 
